...
public class LatokenAccountServiceRaw extends LatokenBaseService {
	...
	/**
	 * Returns all balances in the account, grouped by wallets.
	 * 
	 * @return
	 * @throws LatokenException
	 * @throws IOException
	 */
	public List<List<LatokenBalance>> getLatokenBalances()
			throws LatokenException, IOException {
		
		return latoken.getBalances(
	 ...
	 * @throws IOException
	 */
	public LatokenBalance getLatokenBalance(Currency currency)
			throws LatokenException, IOException {
		
		return latoken.getBalance(
				LatokenAdapters.toSymbol(currency),
				System.currentTimeMillis(),
				apiKey,
				signatureCreator);
	}
